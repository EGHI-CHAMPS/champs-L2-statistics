...
In our examples below, we will often compute statistics for all specimen types excluding nasopharyngeal and oropharyngeal swab, as well as all specimen types. We will specify these as `specimen_types1` and `specimen_types2`:
...
Also note that cases in the "Contributing (P2)" category are found using the ICD10 codes provided to the function. There are only a small subset of pathogens that this categorization into "Contributing (P2)" can occur based only on ICD-10 codes, all others would need the ability to search in the free text field which is not part of the L2 dataset.
